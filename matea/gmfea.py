        ...
        if gen > self.phi:
            if gen % self.theta == 0:
                ...
                for k in range(self.K):
                    ...
                    sorted_by_task = sorted(
                        population, key=lambda x: x.task_fitness[k])
                    m_k = np.zeros(D_max)
            ...
            if len(self.translations) > 0:
                for individual in population:
                    ...
                    new_value = individual.value + \
                        self.translations[individual.skill_factor]
...
class DecisionVariableShufflingStrategy:
    ...
    def shuffle(self, tasks: list[Task], population: list[Individual], p1: Individual, p2: Individual):
        D1 = tasks[p1.skill_factor].dimensions
        D2 = tasks[p2.skill_factor].dimensions
        D_max = max(D1, D2)
        L1 = list(range(D_max))
        L2 = list(range(D_max))
        if D1 == D2:
        ...
        if D1 < D2:
            same_skill_factors = [
                p for p in population if p.skill_factor == p2.skill_factor]
            ...
            same_skill_factors = [
                p for p in population if p.skill_factor == p1.skill_factor]
            p = random.sample(same_skill_factors, 1)[0].copy()
            random.shuffle(L2)
            for i in range(D2):
